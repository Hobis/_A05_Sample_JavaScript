...
// :: 캔버스 쉐이프오프젝트 회전
function p_canvas_shapeObjRotate_core(oso:Object, dir:String):void
{
	var t_nso:Object = null;
	var t_ncos:Array = null;
	
	var t_aco:Object = null;
	var t_nco:Object = null;
	
	var t_la:uint, i:uint;
	
	
	t_nso = p_canvas_shapeObjCopy(oso);
	t_ncos = t_nso.cos;
	t_aco = oso.cos[2];
	for (i = 0, t_la = t_ncos.length; i < t_la; i++)
	{
		t_nco = t_ncos[i];
		if (dir == 'l')
		{
			t_nco = p_canvas_getCellObjAxisOfRotate_left(t_aco, t_nco);
		}
		else if (dir == 'r')
		{
			t_nco = p_canvas_getCellObjAxisOfRotate_right(t_aco, t_nco);			
		}
		t_ncos[i] = t_nco;
	}
	p_canvas_shapeObjCheckDraw(t_nso, oso);
}

// :: 캔버스 쉐이프오프젝트 회전 (90_Angle_Left)
function p_canvas_shapeObjRotate_left(oso:Object):void
{
	p_canvas_shapeObjRotate_core(oso, 'l');
}

// :: 캔버스 쉐이프오프젝트 회전 (90_Angle_Right)
function p_canvas_shapeObjRotate_right(oso:Object):void
{
	p_canvas_shapeObjRotate_core(oso, 'r');
}

// :: 캔버스 쉐이프오프젝트 체크후 표시하기 @Param(cso: CopyShapeObj, oso: OriginalShapeObj)
...
// :: 캔버스 쉐이프오프젝트 이동 (Right) @Param(oso: OriginalShapeObj)
...
// :: 캔버스 쉐이프오프젝트 이동 (Up) @Param(oso: OriginalShapeObj)
...
// :: 캔버스 쉐이프오프젝트 이동 (Down) @Param(oso: OriginalShapeObj)
...
// :: 캔버스 쉐이프오프젝트 표시해도 괜잖은지 확인 @Param(so: ShapeObj)
...
// :: 캔버스 쉐이프오브젝트 표시 해제 @Param(so: ShapeObj)
...
// :: 캔버스 쉐이프오브젝트 표시 @Param(so: ShapeObj)
...
// :: 캔버스 쉐이프오프젝트 보기 좋게 출력 @Param(tso: TargetShapeObj)
function p_canvas_shapeObjPrint(tso:Object):void
{
	var t_str:String =
		'tn=' + tso.tn + ', ' +
		'cos=' + tso.cos;
...
// :: 캔버스 쉐이프오브젝트 생성 @Param(tn: TypeNum[1, 2, 3, 4, 5, 6, 7] 모두 7개 모양의 쉐이프, dp: DefaultPosition)
...
// :: 캔버스 셀 축회전 (aco을 기준으로 tco를 left 회전) @Param(aco: AxisCellObj, tco: TargetCellObj)
function p_canvas_getCellObjAxisOfRotate_left(aco:Object, tco:Object):Object
{
	...
	var t_rv:Object = p_canvas_cellObjCopy(tco);
	t_rv.hn = uint(Math.ceil(aco.hn + GeomUtil.getX(t_nr, t_nd)));
	t_rv.vn = uint(Math.ceil(aco.vn + GeomUtil.getY(t_nr, t_nd)));
	return t_rv;
}

// :: 캔버스 셀 축회전 (aco을 기준으로 tco를 left 회전) @Param(aco: AxisCellObj, tco: TargetCellObj)
function p_canvas_getCellObjAxisOfRotate_right(aco:Object, tco:Object):Object
{
	var t_nd:Number = GeomUtil.getDistance(aco.hn, aco.vn, tco.hn, tco.vn);//NowDistance
	var t_nr:Number = GeomUtil.getRadian2(aco.hn, aco.vn, tco.hn, tco.vn);//NowRadian
	t_nr += GeomUtil.getAToR(90);
...
//		@Param(hn: HorizontalNum, vn: VerticalNum, sn: StateNum)
...
// :: 캔버스 셀을 변경해도 괜잖은지 여부 @Param(hn: HorizontalNum, vn: VerticalNum)
...
// :: 캔버스 셀 상태변경 (CellPoint 사용) @Param(hn: HorizontalNum, vn: VerticalNum, sn: StateNum)
